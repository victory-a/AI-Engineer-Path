...
async function splitDocument(filePath) {
  try {
    ...
    const splitter = new RecursiveCharacterTextSplitter({
      chunkSize: 250,
      chunkOverlap: 35, // aim for 10% overlap
    });
...
async function findNearestMatch(embedding, matchCount = 1, matchThreshold = 0.5) {
  try {
    ...
    const matches = data.map((item) => item.content).join(' \n ');
    return matches;
  } catch (error) {
...
async function getChatResponse(context, question) {
  const chatMessages = [
    {
      role: 'system',
      content: `You are an enthusiastic movie expert who loves recommending movies to people. You will be given two pieces of information - some context about movies and a question. Your main job is to formulate a short answer to the question using the provided context, you can provide multiple answers. If you are unsure and cannot find the answer in the context, say, "Sorry, I don't know the answer." Please do not make up the answer.`,
    },
  ];
...
async function main(input) {
  try {
    ...
    const match = await findNearestMatch(embedding, 3);
